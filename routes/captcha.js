...
router.get('/image', function(req, res, next) {
  ...
  redisClient.setexAsync(captchaKey(captchaId), 600, captchaCode).then(function() {
    res.json({
      captchaId: captchaId,
...
router.post('/verify', function(req, res, next) {
  redisClient.getAsync(captchaKey(req.body.captchaId)).then(function(captchaCode) {
    if (captchaCode && captchaCode === req.body.captchaCode) {
      // 在验证成功后删除验证码信息，防止验证码被反复使用
      return redisClient.delAsync(captchaKey(req.body.captchaId)).then(function() {
